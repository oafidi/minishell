...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/18 12:43:05 by yettalib          #+#    #+#             */
/*   Updated: 2025/08/18 12:44:05 by yettalib         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

static void	handle_var_expand(char *target, t_expand_ctx *ctx,
			t_global_struct *g)
{
	char	*var_exp;
...
char	*expand(char *line, \
			t_global_struct *global_struct, int start_quote_state)
{
	t_expand_ctx	ctx;
...
static int	is_export_case(char *line, t_global_struct *global_struct)
{
	char	*expanded;
	int		result;
	char	*temp;
	...
	while (expanded[i] && is_space(expanded[i]))
		i++;
	if (ft_strncmp(expanded + i, "export", 6) == 0 \
		&& (is_space(expanded[i + 6]) || expanded[i + 6] == '\0'))
		result = 1;
	free(expanded);
	return (result);
}
